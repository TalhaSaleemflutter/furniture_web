...
class ConfirmationDialogMobile extends StatefulWidget {
  final OrderModel order;

  const ConfirmationDialogMobile({super.key, required this.order});

  @override
  State<ConfirmationDialogMobile> createState() =>
      _ConfirmationDialogMobileState();

  /// method to show dialog
  static Future<void> show(BuildContext context, OrderModel order) {
    return showDialog(
      context: context,
      builder: (context) => ConfirmationDialogMobile(order: order),
    );
  }
}
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Text(value, style: AppTextStyles.mobBodyText3(context)),
          ),
        ],
      ),
    );
